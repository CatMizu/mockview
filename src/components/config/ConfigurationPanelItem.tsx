...
export const ConfigurationPanelItem: React.FC<ConfigurationPanelItemProps> = ({
  ...
  return (
    <div className="relative inline-block">
      {/* 主按钮：圆形的音视频开关按钮 */}
      <TrackToggle
        className="flex items-center justify-center w-20 h-14 rounded-full bg-gray-300 hover:bg-gray-400 border border-gray-400 shadow-lg text-gray-800"
        source={
          deviceSelectorKind === "audioinput"
            ...
            <div
              ...
              className={`${
                device.deviceId === deviceSelect.activeDeviceId
                  ? "text-white bg-gray-500"  // 当前设备为深灰色
                  : "text-gray-700"
              } bg-base-100 text-sm py-2 px-4 cursor-pointer hover:bg-gray-300 hover:text-gray-900 rounded-lg`}  // hover时为浅灰色
            >
              {device.label}
            </div>
          );
